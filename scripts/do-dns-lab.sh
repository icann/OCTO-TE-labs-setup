...
case "$arg" in
  all)
    ...
    ;;
  ''|*[!0-9]*)
    echo "Usage: $0 [all|NUMBER]"
    exit 1
    ;;
  *)
    if (( arg < 1 || arg > NETWORKS )); then
      echo "Error: number must be between 1 and $NETWORKS"
      exit 1
    fi
    start=$arg
    end=$arg
